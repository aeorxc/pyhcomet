...
def create_template(assays: list, percent: dict, region: str, productPrices: dict, blendPurchasesPrices: dict, processPurchasesPrices: dict, Units: dict, country: str, name: str, slateorblend: int, type: str):
    settings = {'SlateID': create_slate.create_and_post_slate(assays, name), 'BlendID': create_blend.create_and_post_blend(assays, percent, name), 'RegionID': region,
                'SimplePriceSetID': create_price_set.create_and_post_price_set(productPrices, blendPurchasesPrices, processPurchasesPrices,
                                  Units, name, region),
                'SimpleRefineryConfigID': refineries.get_refinary_id_on_country(region, country, type),
...
def create_and_post_case(assays: list, percent: dict, region: str, productPrices: dict, blendPurchasesPrices: dict, processPurchasesPrices: dict, Units: dict, country: str, name: str, slateorblend: int, type: str):
    x = cases.get_cases()
    try:
        caseid = int(x.query('Name == @name')['ID'].iloc[0])
        cases.put_case(caseid, create_template(assays, percent, region, productPrices, blendPurchasesPrices, processPurchasesPrices, Units, country, name, slateorblend, type))
        return caseid
    except:
        cases.post_case(create_template(assays, percent, region, productPrices, blendPurchasesPrices, processPurchasesPrices, Units, country, name, slateorblend, type))
        x = cases.get_cases()
        caseid = int(x.query('Name == @name')['ID'].iloc[0])
        return caseid
